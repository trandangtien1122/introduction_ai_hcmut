...
class State: 
    ...
    def triggledAt(self, list1: list):
        ...
        '''
        if self.checkConnectToCenter(list1[0], list1[1]):
    ...
    def bumpWater(self, oy=2,ox=2):
        ...
        while len(queue) != 0:
            ...
            j = node_O[1]
            if j < 4:
                if self.checkConnect(self.head[i][j], self.head[i][j+1], "right") == 1 and not self.head[i][j+1]["bump"]:
                    self.head[i][j+1]["bump"] = True
                    queue.append([i, j + 1])
            if j > 0:
                if self.checkConnect(self.head[i][j],self.head[i][j-1], "left") == 1 and not self.head[i][j-1]["bump"]:
                    self.head[i][j-1]["bump"] = True
                    queue.append([i, j - 1])
            if i < 4:
                if self.checkConnect(self.head[i][j],self.head[i+1][j], "up") == 1 and not self.head[i+1][j]["bump"]:
                    self.head[i+1][j]["bump"] = True   
                    queue.append([i+1, j])    
            if i > 0:        
                if self.checkConnect(self.head[i][j],self.head[i-1][j], "down") == 1 and not self.head[i-1][j]["bump"]:
                    self.head[i-1][j]["bump"] = True
                    queue.append([i-1, j])   
    ...
    def checkConnectToCenter(self, oy, ox):
        ...
        while len(queue) != 0:
            ...
            if j < 4: 
                if self.checkConnect(self.head[i][j], self.head[i][j + 1], "right"):
                    if [i, j + 1] not in visited and [i, j + 1] not in queue:
                        queue.append([i, j + 1])
            if j > 0:
                if self.checkConnect(self.head[i][j], self.head[i][j - 1], "left"):
                    if [i, j - 1] not in visited and [i, j - 1] not in queue:
                        queue.append([i, j - 1])
            if i < 4:
                if self.checkConnect(self.head[i][j], self.head[i+1][j], "up"):
                    if [i+1,j] not in visited and [i + 1, j] not in queue:
                        queue.append([i + 1, j])
            if i > 0:
                if self.checkConnect(self.head[i][j], self.head[i - 1][j], "down"):
                    if [i - 1, j] not in visited and [i - 1, j] not in queue:
                        queue.append([i - 1, j])
        return False
    ...
    def expand(self, current: list, parent: list):
        '''
        lấy các ống có connect liền kề ko trùng parrent
        current [oy,ox]
        parrent [oy, ox]
        Trả về list các vị trí liền kề có connect, ko trùng parrent
        '''
        ...
        for item in list1:
            if item[1] > current[1]:
                if self.checkConnect(self.head[current[0]][current[1]], self.head[item[0]][item[1]], "right"):
                    ans.append(item)
            elif item[1] < current[1]:
                if self.checkConnect(self.head[current[0]][current[1]], self.head[item[0]][item[1]], "left"):
                    ans.append(item)
            elif item[0] > current[0]:
                if self.checkConnect(self.head[current[0]][current[1]], self.head[item[0]][item[1]], "up"):
                    ans.append(item)               
            elif item[0] < current[0]:
                if self.checkConnect(self.head[current[0]][current[1]], self.head[item[0]][item[1]], "down"):
                    ans.append(item)
        return ans
    ...
    def stopWaterInValve(self, oy, ox):
        ...
        while len(queue) != 0:
            ...
            queue.pop(0)
            visited.append([i, j])
            if self.head[i][j]["bump"] and not self.checkConnectToCenter(i, j):
                self.head[i][j]["bump"] = False
                queue1 = self.getPosiblePosition(i,j)
                for temp in queue1:
                    if self.head[temp[0]][temp[1]]["bump"]:
...
class Node:
    def __init__(self, matrix: list, rotate: list, previous) -> None:
        ...
        # vị trí thay đổi so với node cũ
        self.rotate = rotate
        if previous is None:
            self.previous = None
            self.step = 0
