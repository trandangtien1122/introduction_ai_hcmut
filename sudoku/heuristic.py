...
class Sudoku:
    ...
    def get_empty_cell(self):
        '''
        Returns an empty cell, sorted by MRV and Degree Heuristic.
        '''
        empty_cells = []
        ...
        for row in range(9):
            for col in range(9):
                if self.board[row][col].value is None:
                    ...
                    empty_cells.append((cell, len(possible_moves), self.get_degree(cell)))

        # Sort empty cells by MRV and Degree Heuristic
        empty_cells.sort(key=lambda x: (x[1], -x[2]))

        # Return the cell with minimum remaining values and maximum degree
        return empty_cells[0][0] if empty_cells else False

    def get_degree(self, cell):
        '''
        Returns the degree of a cell, i.e., the count of empty cells in its row, column, and box.
        '''
        degree = 0

        # Count empty cells in the same row
        for col in range(9):
            if self.board[cell.row][col].value is None and col != cell.col:
                degree += 1

        # Count empty cells in the same column
        for row in range(9):
            if self.board[row][cell.col].value is None and row != cell.row:
                degree += 1

        # Count empty cells in the same box
        for row in range(cell.row // 3 * 3, cell.row // 3 * 3 + 3):
            for col in range(cell.col // 3 * 3, cell.col // 3 * 3 + 3):
                if self.board[row][col].value is None and (row != cell.row or col != cell.col):
                    degree += 1
